// TODO: try to place next to middleware
// search middleware
export const SEARCH_SET_OFFSET = 'SEARCH_SET_OFFSET';
export const SEARCH_START = 'SEARCH_START';
export const SEARCH_END = 'SEARCH_END';
export const SEARCH_ERROR = 'SEARCH_ERROR';
export const TERMINATE_SEARCH = 'TERMINATE_SEARCH';

export const SEARCH_REQUEST = 'SEARCH_REQUEST';
export const SEARCH_REQUEST_SUCCESS = 'SEARCH_REQUEST_SUCCESS';
export const SEARCH_REQUEST_FAIL = 'SEARCH_REQUEST_FAIL';
export const SEARCH_REQUEST_REFUSE = 'SEARCH_REQUEST_REFUSE';

// search by items middleware
export const SEARCH_BY_ITEMS_SET_INDEX = 'SEARCH_BY_ITEMS_SET_INDEX';
export const SEARCH_BY_ITEMS_START = 'SEARCH_BY_ITEMS_START';
export const SEARCH_BY_ITEMS_END = 'SEARCH_BY_ITEMS_END';
export const SEARCH_BY_ITEMS_ERROR = 'SEARCH_BY_ITEMS_ERROR';
export const SEARCH_BY_ITEMS_TERMINATE = 'SEARCH_BY_ITEMS_TERMINATE';

export const SEARCH_BY_ITEMS_REQUEST = 'SEARCH_BY_ITEMS_REQUEST';
export const SEARCH_BY_ITEMS_REQUEST_SUCCESS = 'SEARCH_BY_ITEMS_REQUEST_SUCCESS';
export const SEARCH_BY_ITEMS_REQUEST_FAIL = 'SEARCH_BY_ITEMS_REQUEST_FAIL';
export const SEARCH_BY_ITEMS_REQUEST_REFUSE = 'SEARCH_BY_ITEMS_REQUEST_REFUSE';

// posts
export const POSTS_RECEIVED = 'POSTS_RECEIVED';
export const SET_POSTS_SORT_ORDER = 'SET_POSTS_SORT_ORDER';
export const SET_POSTS_FILTER_TEXT = 'SET_POSTS_FILTER_TEXT';

export const FETCH_WALL_POSTS_REQUEST = 'FETCH_WALL_POSTS_REQUEST';
export const FETCH_WALL_POSTS_FAIL = 'FETCH_WALL_POSTS_FAIL';

// auth
export const SAVE_AUTH_DATA = 'SAVE_AUTH_DATA';
export const SET_USER_NAME = 'SET_USER_NAME';
export const FETCH_USER_NAME_FAIL = 'FETCH_USER_NAME_FAIL';
export const SIGN_OUT = 'SIGN_OUT';
export const NO_VALID_TOKEN = 'NO_VALID_TOKEN';
export const ACCESS_TOKEN_ERROR = 'ACCESS_TOKEN_ERROR';

// redirect
export const REDIRECT_TO_AUTH = 'REDIRECT_TO_AUTH';
export const REJECT_AUTH_OFFER = 'REJECT_AUTH_OFFER';
export const OFFER_AUTH_REDIRECT = 'OFFER_AUTH_REDIRECT';
