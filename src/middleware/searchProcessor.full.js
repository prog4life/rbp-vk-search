    ...
    scannerIntervalId = setInterval(() => {
      ...
      if (reqs.length > 0) {
        ...
        reqs.forEach((req) => {
          ...
          const { id, startTime, attempt } = req;
          const elapsed = Date.now() - startTime;

          if (elapsed > waitTimeout) {
            console.warn(`EXPIRED PENDING with offset: ${req.offset} and attempt ${attempt}, ELAPSED: ${elapsed}`);
            next({
              type: 'REQUEST_FAIL',
              id,
              offset: req.offset,
        ...
        if (nextOffset > total) { // TODO: add !total ?
          ...
          // to remove requests that have exceeded their max attempt limit
...
//     and attempt: ${belated.attempt + 1}`);
//   makeCallToAPI(belated.offset, belated.attempt + 1);
//   return;
// }
...
function checkBelated(reqs, waitTimeout) {
  ...
  //   && belated.attempt < maxAttemptsPending
  // ));
  return reqs.find((req) => {
    ...
    if (req.isPending
      ...
      console.warn(`EXPIRED PENDING with offset: ${req.offset} and attempt ${req.attempt}, ELAPSED: ${elapsed}`);
      return true;
    }
    return false;
  });
